...
impl ProcessBuilder {
  ...
  /// Gets all environment variables explicitly set or unset for the process
  /// (not inherited vars).
  ...
  /// Runs the process, waiting for completion, and mapping non-success exit
  /// codes to an error.
  ...
  /// best of our ability. One aspect we fix here is that we specify a handler
  /// for the Ctrl-C handler. In doing so (and by effectively ignoring it) we
  /// should emulate proxying Ctrl-C handling to the application at hand,
  /// which will either terminate or handle it itself. According to
  /// Microsoft's documentation at <https://docs.microsoft.com/en-us/windows/console/ctrl-c-and-ctrl-break-signals>.
  /// the Ctrl-C signal is sent to all processes attached to a terminal, which
  /// should include our child process. If the child terminates then we'll
  /// reap them in Cargo pretty quickly, and if the child handles the signal
  /// then we won't terminate (and we shouldn't!) until the process itself
  /// later exits.
  pub fn exec_replace(&self) -> Result<()> {
    imp::exec_replace(self)
  }

  /// Executes the process, returning the stdio output, or an error if non-zero
  /// exit status.
...
impl ProcessError {
  ...
  /// * `output` can be `None` if the process did not launch, or output was not
  ///   captured.
  ...
  ///
  /// * `code` can be `None` for situations like being killed by a signal on
  ///   unix.
  pub fn new_raw(
    msg: &str,
    code: Option<i32>,
